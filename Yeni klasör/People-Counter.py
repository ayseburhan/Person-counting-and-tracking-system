...
while True:
    ...
    # imgGraphics = cv2.imread("PeopleCounter/arrows.jpg", cv2.IMREAD_UNCHANGED)
    # imgGraphics_resized = cv2.resize(imgGraphics, (150, 250))

    # img = cvzone.overlayPNG(img, imgGraphics_resized)  # x ve y koordinatlarına göre düzenleyin
    ...
    for result in resultsTracker:
        ...
        if limitsUp[0] < cx < limitsUp[2] and limitsUp[1] - 15 < cy < limitsUp[2] + 15:
            if TotalCountUp.count(Id) == 0:
                TotalCountUp.append(Id)
                cv2.line(img, (limitsUp[0], limitsUp[1]), (limitsUp[2], limitsUp[3]), (0, 255, 0), 5)
    
        if limitsDown[0] < cx < limitsDown[2] and limitsDown[1] - 15 < cy < limitsDown[2] + 15:
